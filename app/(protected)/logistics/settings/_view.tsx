...
import { CheckCircle2, ExternalLink, MapPin, MapPinned, RefreshCw, Save, Truck, X, XCircle } from 'lucide-react';
...
export default function LogisticsSettings() {
  ...
  const handleSave = async () => {
    ...
    if (!name) { setToast({ tone: 'error', message: 'กรุณาระบุชื่อทีมขนส่ง', id: ++toastId.current }); return; }
    ...
    try {
      ...
      clearApiCache();
      setToast({ tone: 'success', message: 'บันทึกข้อมูลทีมขนส่งสำเร็จแล้ว', id: ++toastId.current });
    } catch (err) {
